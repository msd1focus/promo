...
public class PaAddLocationBackingBean implements Serializable {
    ...
    public PaAddLocationBackingBean() {
        super();
    }
        ...
        if (this.getSelectedLocation().size() > 0) {
            for (int i = 0; i < getSelectedLocation().size(); i++) {
                ...
                DCIteratorBinding iterAllCust =
                    (DCIteratorBinding)binding.get(ALL_LOC_ITERATOR);
                for(Row r : iterAllCust.getAllRowsInRange()){
                    if(getSelectedLocation().get(i).equals(r.getAttribute("LocCode"))){   
                    row.setAttribute("Locdescription", r.getAttribute("LocLabel"));
                    }
                }
