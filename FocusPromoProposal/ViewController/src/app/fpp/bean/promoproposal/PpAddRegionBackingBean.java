...
public class PpAddRegionBackingBean implements Serializable {
    ...
    private final String SELECTED_REGION_ITERATOR = "PropRegionView1Iterator";
    private final String ALL_REGION_ITERATOR =
        "AllProposalRegionShuttleView1Iterator";
    List unfilteredList;

    private boolean resetShuttle = true; //false;

    public PpAddRegionBackingBean() {
        super();
    }
    ...
    public String processShuttle() {
        ...
        if (this.getSelectedRegions().size() > 0) {
                for (int i = 0; i < getSelectedRegions().size(); i++) {
                    ...
                    DCIteratorBinding iterAllRegion =
                        ADFUtils.findIterator(ALL_REGION_ITERATOR);
                    for(Row r : iterAllRegion.getAllRowsInRange()){
                    if(getSelectedRegions().get(i).equals(r.getAttribute("RegionCode").toString())){
                        row.setAttribute("Description1", r.getAttribute("RegionLabel"));
                          }
                    }
